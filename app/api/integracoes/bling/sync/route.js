...
import { getAuthProfile, ensureToken, fetchContas, mapConta } from '@/lib/bling-server'
...
export async function POST(request) {
  ...
  const { integracao_id, modulo = 'fluxo', fase = 0, pagina = 1, diag = false } = await request.json()
  ...
  try {
    ...
    const itens = await fetchContas(integ, recurso, pagina)
    ...
    for (const item of itens) {
      const { registro, faltantes } = mapConta(item, { tipoFluxo, empresaId: integ.empresa_id })
