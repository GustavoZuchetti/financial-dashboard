...
export async function POST(req) {
  try {
    ...
    // Buscar todos os registros do fluxo_caixa com paginação
    let all = [], pg = 0
    while (true) {
    ...
    // Calcular e fazer upsert de cada mês
    ...
    for (const v of Object.values(porMes)) {
      ...
      await supabaseAdmin.from('ciclo_financeiro').upsert({
        empresa_id,
        ano:                    v.ano,
        mes:                    v.mes,
        pmr,
        ...
        ciclo_financeiro_valor: cf,
      }, { onConflict: 'empresa_id,ano,mes' })

      mesesProcessados++
    }

    return NextResponse.json({ ok: true, meses: mesesProcessados, total: all.length })
  } catch (e) {
