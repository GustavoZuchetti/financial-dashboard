...
| 14/jul | AUTOMAÇÃO VALIDADA DE PONTA A PONTA: cron diário 06:00 BRT com cursor persistente em colunas dedicadas (cron_cursor/cron_resultado/ultima_sync_cron). Saga de 3 causas-raiz resolvidas: (1) auth do cron exigia Bearer que a Vercel não envia → aceita x-vercel-cron; (2) colunas não existiam (migração nunca aplicada + .catch silencioso) + schema cache do PostgREST exigiu NOTIFY reload; (3) NEXT.JS DATA CACHE congelava TODOS os SELECTs do supabase-js server-side (fetch GET cacheado por padrão) → getAdmin() com cache:'no-store'. Prova final: cursor retomando 31→41→50 com relido=final e erro_gravacao=null. Token Management API renovado (sbp_3b98...25c6 na Vercel). Gestão: painel Indicadores do Período (Recebido/A Receber/Pago/A Pagar/Atrasos) no lugar do saldo acumulado |
| 03/jul | Integração via API (Bling): liberação por módulo no portal ADM (organizations.api_dre_liberado/api_fluxo_liberado), tabela integracoes (1 por entidade, RLS deny-all/service-role), OAuth v3 (authorize_url assinada + callback), sync paginada com cursor e modo diagnóstico (?diag), conciliação por doc_ref (unique em fluxo_caixa e lancamentos), aba Configurações→Integrações, trava anti-duplicidade na Importação por arquivo. GO-LIVE 03/07: FACE conectada, 4.942 títulos sincronizados (2023→2036), origem 100% API, status reais (2.605 abertos / 2.334 pagos / 3 parciais). Limitação conhecida: listagem do Bling não traz data de liquidação (2.334 pagos sem data — enriquecimento por detalhe é evolução futura). Módulo DRE aguarda revisão do De-Para |
