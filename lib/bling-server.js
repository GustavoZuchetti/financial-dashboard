...
export async function fetchDetalhe(integ, recurso, id) {
  const r = await fetch(`${BLING.API_BASE}/${recurso}/${id}`, {
...
// Nome de um contato (a listagem de contas a pagar vem sem contato.nome)
export async function fetchContatoNome(integ, id) {
  try {
    const r = await fetch(`${BLING.API_BASE}/contatos/${id}`, {
      headers: { 'Authorization': `Bearer ${integ.access_token}`, 'Accept': 'application/json' } })
    const body = await r.json().catch(() => null)
    if (!r.ok) return null
    return body?.data?.nome || null
  } catch { return null }
}
...
export async function fetchBorderoData(integ, borderoId) {
  try {
    const r = await fetch(`${BLING.API_BASE}/borderos/${borderoId}`, {
      headers: { 'Authorization': `Bearer ${integ.access_token}`, 'Accept': 'application/json' } })
    const body = await r.json().catch(() => null)
    if (!r.ok) return null
    const d = body?.data
...
export function mapConta(item, { tipoFluxo, empresaId }) {
  ...
  const registro = {
    ...
    descricao:       pick(item, ['contato.nome', 'historico', 'descricao', 'observacoes'])
