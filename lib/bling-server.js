...
export async function fetchCategoriaNome(integ, id) {
  try {
    const r = await fetch(`${BLING.API_BASE}/categorias/receitas-despesas/${id}`, {
      headers: { 'Authorization': `Bearer ${integ.access_token}`, 'Accept': 'application/json' } })
    const body = await r.json().catch(() => null)
    return body?.data?.descricao || body?.data?.nome || null
  } catch { return null }
}
...
export function tituloCompleto(e, hoje = new Date().toISOString().split('T')[0]) {
  ...
  // aberto/parcial já vencido pode ter sido pago no Bling → força reconferência
  const statusPodeMudar = ['aberto','parcial'].includes(e.status) && e.data <= hoje
...
export async function montarRegistrosFluxo(admin, integ, recurso, tipoFluxo, pagina, limite, categoriasMap, nomesContato, itensPre = null) {
  ...
  const { data: existentes } = await admin.from('fluxo_caixa')
    .select('doc_ref,descricao,categoria,competencia,status,data,data_liquidacao').in('doc_ref', docRefs)
  const completo = {}
  ;(existentes || []).forEach(e => { completo[e.doc_ref.split(':').pop()] = tituloCompleto(e) })
  ...
  for (const item of itens) {
    ...
    // Data de liquidação via borderô — para pago E parcial (a parte liquidada)
    if (['pago','parcial'].includes(registro.status) && !registro.data_liquidacao
