...
// Índice categoria → natureza. Ausência significa `operacional`: enquanto o
// Controller não classificar, nada muda.
export function indexarNaturezas(linhas) {
  const mapa = new Map()
  for (const l of linhas || []) {
    const k = norm(l?.categoria)
    if (k) mapa.set(k, l.natureza || NATUREZA_PADRAO)
  }
  return mapa
}
