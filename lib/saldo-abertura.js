...
// ─── CONSOLIDAÇÃO ───────────────────────────────────────────────────────────
// O consolidado é SEMPRE a soma de âncoras certificadas por entidade, nunca uma
// âncora única rateada. Gravar um saldo de grupo numa entidade infla a tela dela
// e zera as demais; ratear por estimativa erra as três.
//
// REGRA DE SILÊNCIO REFORÇADA: se UMA entidade sequer não tiver âncora vigente,
// o total consolidado é NULO. Um consolidado parcial subestima o caixa e é
// exatamente o tipo de número que não pode chegar a CEO e investidores.
//
// entidades: [{ empresa_id, nome, ancoras: [...] }]
export function consolidarAncoras({ entidades, data }) {
  const linhas = [], faltando = []
  let total = 0
  for (const e of entidades || []) {
    const a = ancoraVigente(e.ancoras, data)
    if (!a) {
      faltando.push({ empresa_id: e.empresa_id, nome: e.nome })
      linhas.push({ empresa_id: e.empresa_id, nome: e.nome, ancora: null, valor: null })
      continue
    }
    total = r2(total + Number(a.valor))
    linhas.push({ empresa_id: e.empresa_id, nome: e.nome, ancora: a, valor: Number(a.valor) })
  }
  return {
    ok: faltando.length === 0 && linhas.length > 0,
    linhas,
    total: (faltando.length === 0 && linhas.length > 0) ? total : null,
    faltando,
    naoCertificadas: linhas.filter(l => l.ancora && !l.ancora.conciliado_em).map(l => l.nome),
  }
}

// Saldo consolidado numa data: soma dos saldos por entidade.
// entidades: [{ empresa_id, nome, ancoras: [...], registros: [...] }]
export function saldoConsolidadoEm({ entidades, data, hoje = hojeISO() }) {
  const linhas = [], faltando = []
  let saldo = 0, entradas = 0, saidas = 0, aproximados = 0, semSinal = 0
  for (const e of entidades || []) {
    const r = saldoEm({ ancoras: e.ancoras, registros: e.registros, data, hoje })
    linhas.push({ empresa_id: e.empresa_id, nome: e.nome, ...r })
    if (!r.ok) { faltando.push({ empresa_id: e.empresa_id, nome: e.nome }); continue }
    saldo = r2(saldo + r.saldo); entradas = r2(entradas + r.entradas); saidas = r2(saidas + r.saidas)
    aproximados += r.aproximados; semSinal += r.semSinal
  }
  const completo = faltando.length === 0 && linhas.length > 0
  return {
    ok: completo, motivo: completo ? null : 'entidade_sem_ancora',
    linhas, faltando,
    saldo: completo ? saldo : null,
    entradas: completo ? entradas : null,
    saidas: completo ? saidas : null,
    aproximados, semSinal,
  }
}

// Saldo de PARTIDA consolidado: posição na abertura de `inicioPeriodo`.
// É a função que as telas de Visão Geral, Fluxo de Caixa e Gestão consomem.
// entidades: [{ empresa_id, nome, ancoras: [...], registros: [...] }]
export function saldoDePartidaConsolidado({ entidades, inicioPeriodo, hoje = hojeISO() }) {
  const linhas = [], faltando = []
  let saldo = 0, entradas = 0, saidas = 0, aproximados = 0, semSinal = 0
  for (const e of entidades || []) {
    const r = saldoDePartida({ ancoras: e.ancoras, registros: e.registros, inicioPeriodo, hoje })
    linhas.push({ empresa_id: e.empresa_id, nome: e.nome, ...r })
    if (!r.ok) { faltando.push({ empresa_id: e.empresa_id, nome: e.nome }); continue }
    saldo = r2(saldo + r.saldo); entradas = r2(entradas + r.entradas); saidas = r2(saidas + r.saidas)
    aproximados += r.aproximados; semSinal += r.semSinal
...
// Agrupa registros por empresa_id — as telas buscam tudo numa consulta só e
// precisam separar por entidade para aplicar a âncora correta de cada uma.
export function agruparPorEmpresa(registros) {
  const mapa = {}
  for (const r of registros || []) {
...
// Monta a lista de entidades no formato que as funções consolidadas esperam.
export function montarEntidades({ empIds, nomes = {}, ancoras = [], registros = [] }) {
  const porEmpresa = agruparPorEmpresa(registros)
  return (empIds || []).map(id => ({
    empresa_id: id,
    nome: nomes[id] || id,
    ancoras: (ancoras || []).filter(a => a.empresa_id === id),
    registros: porEmpresa[id] || [],
  }))
}
