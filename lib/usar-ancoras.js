...
export function useAncoras(empIds) {
  ...
  useEffect(() => {
    let cancelado = false
    // Volta a "carregando" a cada troca de entidades: sem isto as telas
    // enxergavam a lista ANTERIOR (ou vazia) como resultado final e compunham
    // o saldo com âncoras que ainda não chegaram.
    setAncoras(null)
    ...
    ;(async () => {
      try {
        ...
        setMigracaoPendente(!!j.migracao_pendente)
  ...
  return { ancoras, erro, migracaoPendente, carregando: ancoras === null }
}

// Mensagem padrão quando o saldo não pode ser composto — usada nas três telas
// para que o texto seja idêntico em todas.
export function motivoIndisponivel(faltando = [], migracaoPendente = false) {
  if (migracaoPendente)
    return 'Migração do saldo de abertura ainda não aplicada no banco. Execute supabase/migrations/20260812_saldos_abertura.sql no SQL Editor do Supabase.'
